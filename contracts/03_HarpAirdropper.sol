...
contract HarpAirdropper is onlyOwner{
  ...
  function sendAmount(address[] memory _user, uint256[] memory _amounts) isOwner public returns(bool){
	for(uint i=0; i< _user.length; i++)
    token.transfer(_user[i], _amounts[i]*10**decimal);
    return true;
  }
